...
export default function FeatureAdminPage() {
  ...
  const toggle = async (accountId: string, featureKey: string) => {
    ...
    setFeatures((current) => [...current.filter((f) => !(f.account_id === accountId && f.feature_key === featureKey)), data.feature!])
  }

  return (
    <div className="mx-auto max-w-5xl space-y-6 p-6">
      <div>
        <p className="text-sm font-medium text-primary">SutraAPI Admin</p>
        <h1 className="mt-1 text-2xl font-semibold text-foreground">Feature access</h1>
        <p className="mt-1 text-sm text-muted-foreground">Grant optional modules to specific client accounts.</p>
      </div>
      ...
      {loaded ? (
        <div className="overflow-hidden rounded-xl border border-border bg-card">
          ...
          {accounts.map((account) => (
            <div key={account.id} className="grid grid-cols-[minmax(0,1fr)_repeat(3,130px)] items-center border-b border-border px-5 py-4 last:border-b-0">
              <span className="truncate text-sm font-medium text-foreground">{account.name}</span>
              {FEATURES.map(([key]) => <button key={key} type="button" onClick={() => void toggle(account.id, key)} className={enabled(account.id, key) ? 'mx-auto rounded-full bg-primary/10 px-3 py-1 text-xs font-medium text-primary' : 'mx-auto rounded-full bg-muted px-3 py-1 text-xs font-medium text-muted-foreground'}>{enabled(account.id, key) ? 'Enabled' : 'Off'}</button>)}
            </div>
          ))}
        </div>
      ) : null}
